...
C***********************************************************************
C
      SUBROUTINE INITAL(NNOD,NSTR,N,NT,NTRI,NP,NQ,NSF,NDIR,NDIRC,NENS,
     1                  HTIDIR,HTINEU,HSPATM,HTIATM,IETO,IPRT1,IPOND,  
...
C
      ...
      INCLUDE 'CATHY.H'
      INTEGER  I,j
      INTEGER  NNOD,NSTR,N,NT,NTRI,NSF,DUPUIT,NENS
      ...
      LOGICAL  DTGMIN,ENKF,SFCHEK,SURF,DEM,PONDING,PONDP
...
c PLANT
      ...
      REAL*8   METEOTIM(*),METEOINP(3,*),METEO(*)
      LOGICAL  PLANT_FLAG
...
C
     ...
     5           NP,NODDIR,CONTP,QPOLD,NSF,NSFNUM,SFQP,
     6           ISFCVG,SFCHEK,HGFLAG,SFFLAG,
...
C
C     IF (NP(3) .NE. 0) THEN
         CALL BCONE('   NATM, NSF DIRICHLET',IIN8,IOUT2,IOUT19,NNOD,
...
C     IF (NQ(3) .NE. 0) THEN
      CALL INIT0I(3,ZERO)
      CALL INIT0I(3,ZEROC)
         CALL BCONE('     NATM, NSF NEUMANN',IIN9,IOUT2,IOUT20,NNOD,
     1              NQMAX,IPRT1,ZERO,ZEROC,NQ,NSTR,HTINEU,TIME,
     2              DELTAT,QTIM,QINP,CONTQ,Q,ANQ,ACONTQ)
C   PLANT
      IF (PLANT_FLAG) THEN
         CALL METEONE('   METEOROLOGICAL DATA',IIN62,IOUT2,
     1              IPRT1,HTIMETEO,TIME,
     2              DELTAT,METEOTIM,METEOINP,METEO,NMETEODATA)
      ENDIF
C     WRITE(223,*) TIME                                       
C     WRITE(223,*) ANP                                        
...
C     END IF
C  
C  Initialize QPOLD, since init1.f doesn't anymore
C
...
C
     ...
     2            ATMINP,PNEW,PTIMEP,
     3            ANP,ANQ,ACONTP,ACONTQ,NSF,NSFNUM,NSFNOD,
     4            NENS,ENKF,DSATM,ENATMPOT,ENATMACT,ENATMOLD,QNEW,
     5            QTIMEP)
C     WRITE(225,*) TIME                                       
C     WRITE(225,*) ANP                                        
...
C
      CALL SFINIT(NSF,NSFNUM,NSFNOD,SFEX,SFEXP,SFEXIT,DUPUIT,
