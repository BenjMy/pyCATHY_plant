...
C
      IF (IVGHU .EQ. 0 .OR. IVGHU .EQ. 1) THEN
         ...
         VGNR=1.0D0/VGN
         VGMM1=VGM-1.0D0
         VGPSN=DABS(VGPSAT)**VGN
         VGMR=1.0D0/VGM
         VGM52=2.5D0*VGM
         IF (IVGHU .EQ. 0) THEN
            DO I=1,N
               VGPNOT(I)=(PNODI(I)-VGRMC)/PNODI(I)
            END DO
         ELSE
C
...
C
            ...
            DO I=1,N
               SS=SNODI(I)
               TSR=PNODI(I)-VGRMC
               V2=VGPSAT*(VGM**(1.0D0/VGN))
               V3=0.0D0
               V4=-VGM*VGN*TSR*(VGM**VGM)/(VGPSAT*(VGM1**VGM1))
               IF (SS .GE. V4) THEN
   ...
   30          K=K+1
                  ...
                  V5=(PNOT/VGPSAT)**VGN
                  V6=(1.0D0+V5)**VGM1
                  V7=((DABS(PNOT)**VGN1)/V6) - (SS*VGPSN/(VGN1*TSR))
                  IF (V7 .EQ. 0.0D0 .OR. (V3-V2)/2.0D0 .LT. V1) GO TO 50
                  V5=(V2/VGPSAT)**VGN
                  V6=(1.0D0+V5)**VGM1
                  V8=((DABS(V2)**VGN1)/V6) - (SS*VGPSN/(VGN1*TSR))
